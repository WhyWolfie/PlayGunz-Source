...
#define MINTERVAL_GARBAGE_SESSION_CLEANING	20*60*1000		// 20 min
	...
	MGetServerStatusSingleton()->SetRunStatus(105);
	if (nGlobalClock - CustomEventTimers[0] >= 1601000)
	{
		if (GetClientCount() > 120)
		{
			...
			{
			case 1:
				numb = MMATCH_GAMETYPE_DEATHMATCH_SOLO;
				break;
			case 3:
				numb = MMATCH_GAMETYPE_BERSERKER;
				break;
			}
			OnCreateEvent(false, "", (MMATCH_GAMETYPE)numb, 25,  10, 16);
			OnCreateEvent(false, "", (MMATCH_GAMETYPE)numb, 25,  10, 16);
			char szMsg[250];
		...
		CustomEventTimers[0] = nGlobalClock;
	}
	if (nGlobalClock - CustomEventTimers[1] >= 5601000)
	{
		if (GetClientCount() > 120)
		{
			int numb = RandomNumber(0, 3);
			...
			OnCreateEvent(false, "", (MMATCH_GAMETYPE)numb, 50,  10, 16);
			OnCreateEvent(false, "", (MMATCH_GAMETYPE)numb, 50,  10, 16);
			char szMsg[250];
			sprintf(szMsg, "%s event has started.", (MGetGameTypeMgr()->IsTeamGame((MMATCH_GAMETYPE)numb) ? "Team" : "Solo"));
			MCommand* pCmd = MMatchServer::GetInstance()->CreateCommand(MC_MATCH_ANNOUNCE, MUID(0, 0));
		...
		CustomEventTimers[1] = nGlobalClock;
	}
