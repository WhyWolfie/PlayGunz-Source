...
bool MMatchServer::OnCommand(MCommand* pCommand)
{
	...
	case MC_STAFF_STAGE_JOIN:
	{
		...
		MMatchObject* pObj = (MMatchObject*)GetObject(pCommand->GetSenderUID());
		if (pObj && IsAdminGrade(pObj) == true || pObj && pObj->GetAccountInfo()->m_nUGrade == MMUG_ADMIN || pObj && pObj->GetAccountInfo()->m_nUGrade == MMUG_JORK || pObj && pObj->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER)
		{
