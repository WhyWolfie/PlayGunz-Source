...
bool MMatchServer::StageJoin(const MUID& uidPlayer, const MUID& uidStage, bool rejoin, MMatchTeam Team)
{
	...
	// Cast Join
	if (rejoin == true && pStage->GetStageType() != MST_LADDER && pStage->GetStageType() != MST_LADDER) rejoin = false;
	if (rejoin == false)
	{
...
void MMatchServer::OnCreateEvent(bool bPrivate, char* pszStagePassword, MMATCH_GAMETYPE Type, int Limit, int reward, int Players)
{
	MMatchChannel* pChannel = FindChannel(MCHANNEL_TYPE_PRESET, "Event");
	if (pChannel == NULL) return;
	MUID uidStage;
	char pszStageName[250];
	sprintf(pszStageName, "%s | %s | %d %s | %d reward", (MGetGameTypeMgr()->IsTeamGame(Type) ? "Team" : "Solo"), Limit, (MGetGameTypeMgr()->IsTeamGame(Type) ? "Rounds" : "Kills"), reward);
	...
	pStage->GetStageSetting()->SetLimitTime(9999);
...
void MMatchServer::OnStageStart(const MUID& uidPlayer, const MUID& uidStage, int nCountdown)
{
	...
	if (pStage->m_bIsEvent == false && strstr(pChan->GetName(), "Event") && pMasterObj && !IsAdminGrade(pMasterObj) || pStage->m_bEventFinnished == true && pStage->m_bIsEvent == true)
	{
		char szMsg[256];
...
void MMatchServer::OnStageTeam(const MUID& uidPlayer, const MUID& uidStage, MMatchTeam nTeam)
{
	...
	if (pStage->GetStageType() == MST_LADDER && !IsAdminGrade(pChar) && !pChar->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER && !pChar->GetAccountInfo()->m_nUGrade == MMUG_EVENTMASTER || pStage->m_bIsEvent && !IsAdminGrade(pChar) && !pChar->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER && !pChar->GetAccountInfo()->m_nUGrade == MMUG_EVENTMASTER || pStage->GetStageType() == MST_LADDER && !IsAdminGrade(pChar) && !pChar->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER && !pChar->GetAccountInfo()->m_nUGrade == MMUG_EVENTMASTER) return;
	if (pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM &&
		pChar->GetTeam() == nTeam)	{
		nTeam = MMT_RED;
	}
...
void MMatchServer::OnVoteCallVote(const MUID& uidPlayer, const char* pszDiscuss, const char* pszArg)
{
	...
	for (MUIDRefCache::iterator itor = pStage->GetObjBegin(); itor != pStage->GetObjEnd(); itor++) {
		...
		if ((pPlayer) && (IsAdminGrade(pPlayer)) || (pPlayer) && pPlayer->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER || (pPlayer) && pPlayer->GetAccountInfo()->m_nUGrade == MMUG_EVENTMASTER)
		{
