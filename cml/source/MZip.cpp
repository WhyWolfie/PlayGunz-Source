...
#define encCode2	0x7
#define encCode3	0x9
#define encCode4	0x7b
#define encCode5	0xea
#endif

#if defined(_ENCRYPT) && defined(_MAX)
...
#elif defined(_ENCRYPT) && !defined(_MAX)
void ConvertChar(char* pData, int _size)
{
	if (!pData) return;
...
#endif  
#else
char key[63] = { 'p', '4', 'C', 'k', 'b', '9', '<', 't', 'M', 'K', '&', '/', '/', '%', 'V', 'Z', '3', '{', 'c', 'g', '~', 't', '=', '*', 'U', ';', 'T', 'Q', '(', 'B', 'M', 'E', 'I', ';', '}', 'z', 'W', ',', '/', 'O', 'K', '6', 'w', '4', '{', 'S', 'L', '7', 's', 'H', '@', '2', 'E', '_', 'l', '0', '&', 'g', '3', '!', 'P', 'Z', 'f' };
...
void ConvertChar(char* pData, int _size)
{
	...
	{
		b = *pData;
		b = b ^ key[i % (sizeof(key) / sizeof(char))];
		*pData = b;
		pData++;
	}
}
...
bool MZip::Initialize(FILE* fp, unsigned long ReadMode)
{
	...
	if (m_nZipMode >= ZMode_Mrs2)							// mrs2 ???? ??? ??..
		RecoveryChar((char*)&dh, sizeof(MZIPDIRHEADER));		// v2 ????..
	...
	if (m_nZipMode >= ZMode_Mrs2)
		RecoveryChar((char*)m_pDirData, dh.dirSize);//mrs ?? ??..
...
bool MZip::ReadFile(int i, void* pBuffer, int nMaxSize)
{
	...
	fseek(m_fp, h.fnameLen + h.xtraLen, SEEK_CUR);// MRS ??? ?? ?? ??..
...
bool MZip::RecoveryMrs2(FILE* fp)
{
	...
	RecoveryChar((char*)pDirData, dir_data_size);//mrs ?? ??..
