...
bool ZGameInterface::OnCommand(MCommand* pCommand)
{
	...
	case MC_MATCH_RESPONSE_BUY_ITEM:
	{
		...
		else if ((nResult == MERR_TOO_EXPENSIVE_BOUNTY) || (nResult == MERR_TOO_MANY_ITEM) || (nResult == MERR_TOO_EXPENSIVE_CASH) || (nResult == MERR_TOO_EXPENSIVE_EVENT)) /// PlayGunZ: <CashShop y EventShop>
		{
			ZApplication::GetGameInterface()->ShowErrorMessage(nResult);
		}
		else
