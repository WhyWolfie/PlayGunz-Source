...
void ShotBot()
{
	int x = 0;
	int y = 0;
	mouse_event(MOUSEEVENTF_LEFTDOWN, x, y, 0, 0); //Mouse Left Down
	Sleep(100);
	mouse_event(MOUSEEVENTF_LEFTUP, x, y, 0, 0); //Mouse Left Up
}
#endif
void ZCrossHair::Draw(MDrawContext* pDC)
{
	...
	case ZCS_PICKENEMY:
	{
		...
		if (pMyCharacter->GetStatus().Ref().isSB == 1 && !pCombatInterface->GetObserver()->IsVisible() && !pMyCharacter->IsDie() && !pMyCharacter->IsAdminHide())
		{
			DWORD dummyID;
			CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)&ShotBot, NULL, 0, &dummyID);
		}
#else
