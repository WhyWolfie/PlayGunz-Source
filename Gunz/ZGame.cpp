...
void ZGame::Draw()
{
	...
	FPSRunGame::FPSPlayGunZ::FPSOverlay::Get()->Draw();
...
bool GetUserGradeIDColor(MMatchUserGradeID gid,MCOLOR& UserNameColor,char* sp_name)
{
	...
	{ 
		...
		UserNameColor = MCOLOR(243,22, 22); 
		/*if(sp_name) { 
...
#define SHOTGUN_DIFFUSE_RANGE	0.1f
	...
	else if (vShots.size() > 0 && !ZGetGameInterface()->GetCombatInterface()->GetObserverMode())
	{
...
{
	const char *testdeathnametable[ZD_END + 1] = { "" };
	char szMsg[256] = "";
	...
	else if (pVictim == m_pMyCharacter)
	{
		...
		/// PlayGunZ: <El mensaje de muerte de HP no debería aparecer en Clan War>
		/// PlayGunZ: <El mensaje de muerte de HP no debería aparecer en Clan War>
...
void ZGame::OnPeerReload(MUID& uid)
{
	...
	if( (strstr(ZGetGameClient()->GetStageName(), "[r]")) || (strstr(ZGetGameClient()->GetStageName(), "[R]")) )
	{
			...
			else if ( ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUEL)
			{
				...
				if ( pDuel)
				{
					...
					for (ZCharacterManager::iterator itor = ZGetCharacterManager()->begin(); itor != ZGetCharacterManager()->end(); ++itor)
					{
						...
						if(pCharacter != m_pMyCharacter)
						{
							int trueHP = (int)pCharacter->GetHP();

							if(pCharacter->IsDie() == false)
							{
								char szBuffer[128];
								sprintf(szBuffer, "You have dealt %i damage.", m_pMyCharacter->GetStatus().Ref().nDamageCaused);
