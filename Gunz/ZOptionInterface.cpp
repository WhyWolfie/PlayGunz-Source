...
/// PlayGunZ: <Support 16:9 aspect ratios>
	...
	*/
	...
	//	ComboBox
	{
		...
		if(pWidget)
		{
			...
			/// PlayGunZ: <Corrección de error del iterador si la resolución no es compatible>
	...
	{	
		...
		if (pComboBox != NULL) {
			if (Z_EXTRA_SWORDTRACKS >= pComboBox->GetCount())
			{
				Z_EXTRA_SWORDTRACKS = 0;
			}
			pComboBox->SetSelIndex(Z_EXTRA_SWORDTRACKS);
			RSetTracksSize(Z_EXTRA_SWORDTRACKS);
		}
		// 1ÃÊ´ç ÇÁ·¹ÀÓ Á¦ÇÑ
...
bool ZOptionInterface::SaveInterfaceOption(void)
{
	...
	*/
	...
	{	// Etc
		...
		if (pComboBox != NULL)
		{
			Z_EXTRA_SWORDTRACKS = pComboBox->GetSelIndex();
			RSetTracksSize(Z_EXTRA_SWORDTRACKS);
		}
		pComboBox = (MComboBox*)pResource->FindWidget("CameraDistance");
		if (pComboBox)
...
#ifndef _MULTILANGUAGE	// ´ÙÁß¾ð¾îÁö¿ø µðÆÄÀÎÀ» ²°À»¶© ¾ð¾î¼±ÅÃ ÄÞº¸¹Ú½º¸¦ ¼û°Ü¹ö¸°´Ù
