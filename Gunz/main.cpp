...
RRESULT OnCreate(void *pParam)
{
	...
	sprintf(cstrReleaseDate, "(GunzTheDuel 1.5) - Build : %s", __DATE__); //No quitar el nombre porque el trabajo me pertenece ni al metro con jorklenis.
	...
	*/
	...
	CreateThread(0, 0, (LPTHREAD_START_ROUTINE)ZProtect28, 0, 0, 0);
#ifdef _ANTISB
	PlayAnti = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)AntiShotbotLogger, 0, 0, 0);
	PlayAnti = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)SearchandDestroy, 0, 0, 0);
#endif
	//PlayAnti = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)PlayProtect, 0, 0, 0);
//	GetRawInputDataDetour = new cDetour(GetProcAddress(LoadLibraryA("user32.dll"), "GetRawInputData"), GetRawInputDataHook);
...
int PASCAL WinMain(HINSTANCE this_inst, HINSTANCE prev_inst, LPSTR cmdline, int cmdshow)
{
	...
	PlayAnti = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)Scanner, 0, 0, 0);
	...
	nRet = _AhnHS_Initialize(szFullFileName, AhnHS_Callback,
		4003, "CC4686AE10F63E4A",
		AHNHS_CHKOPT_ALL | AHNHS_SELFDESTRUCTION_RATIO_FAST | AHNHS_CHKOPT_LOADLIBRARY | AHNHS_CHKOPT_PROTECT_D3DX | AHNHS_CHKOPT_LOCAL_MEMORY_PROTECTION, // hidden memory
		AHNHS_SPEEDHACK_SENSING_RATIO_HIGHEST);

	if (nRet != HS_ERR_OK)
	{
		mlog("HackShield Initialize Error. Folder HShield is missing from your computer(Error Code = %x).", nRet);
		return FALSE;
	}
	...
	if (nRet != HS_ERR_OK)
	{
		mlog("HackShield Start Error. Folder HShield is missing from your computer(Error Code = %x).", nRet);
		return FALSE;
	}
	...
	// 로그 시작
	mlog("PLAYGUNZ " STRFILEVER " launched. build ("__DATE__" "__TIME__") \n");
	char szDateRun[128] = "";
	char szTimeRun[128] = "";
	_strdate(szDateRun);
	_strtime(szTimeRun);
...
#ifdef _ZPROTECT28 //hidden address
int __stdcall AhnHS_Callback(long lCode, long lParamSize, void* pParam)
{
	switch (lCode)
	{
