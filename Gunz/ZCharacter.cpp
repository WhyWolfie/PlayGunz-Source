	...
	for (int i = 0; i < MMCIP_END; i++) {
		...
		else if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_MODE_STAFF)
		{
			...
			if (GetProperty()->nSex == MMS_MALE){
				...
				m_Items.EquipItem(MMCIP_CHEST, 5060205);
				...
				m_Items.EquipItem(MMCIP_CHEST, 5060210);
			...
			m_Items.EquipItem(MMCIP_CUSTOM2, 900014);
		}
...
#define UPDATE_HEALTH_APV 1
	...
	if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_VAMPIRE && pAttacker == ZGetGame()->m_pMyCharacter)
	{
