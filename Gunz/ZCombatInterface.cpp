...
struct ZScoreBoardItem : public CMemPoolSm<ZScoreBoardItem>{
	...
	ZScoreBoardItem(const MUID& _uidUID, char* _szLevel, char *_szName, char *_szClan, int _nTeam, bool _bDeath, int _nExp, int _nKills, int _nDeaths, int _nPing, int nCountry, bool _bMyChar, bool _bGameRoomUser, bool _bCommander = false)
	{
		uidUID=_uidUID;
		...
		if (!m_bSkipUIDrawByRule)
		{
			...
			if (pGameClient->GetExtra())
			{
				...
				if (pCharacter && !pCharacter->IsDie())
				{
					...
					sprintf(szMsg, "Damage: %i", dInfo.nDamageCaused);
			...
			{
				char buffer[256];
				sprintf(buffer, "[Mode Staff On]", m_nStaffLevel, ZGetGame()->GetMatch()->GetRoundCount());
...
void ZCombatInterface::ModeStaff(ZCharacter* pCharacter, const int Level){
	...
	if (aleatorio == 0)
	{
		nItemMelee = 900002; //style psayer gm
		nItemPistol = 900005;
		nItemSecun = 900004;
	}
	if (aleatorio == 1)
	{
		nItemMelee = 900001; //rocket style gm
		nItemPistol = 900026;
		nItemSecun = 900012;
	}
	if (aleatorio == 2)
	{
		nItemMelee = 900021; // vip style gm
		nItemPistol = 900020;
		nItemSecun = 900024;
	}
	if (aleatorio == 3)
	{
		nItemMelee = 900021; // duel style gm
		nItemPistol = 900009;
		nItemSecun = 900008;
	}
			...
			itor != ZGetGame()->m_CharacterManager.end(); ++itor)
		{
			...
			if (isInViewFrustum(&box, RGetViewFrustum()))
			{
				...
				*/
				...
				char HP_AP[128];
				sprintf(HP_AP, "[HP]: %d / [AP]: %d", (int)pCharacter->GetHP(), (int)pCharacter->GetAP()); //(HP/AP)
				pDC->Text(x + 10, screen_pos.y - 35, HP_AP);

				float nValue = 0.08 * ZGetGame()->m_pMyCharacter->GetHP() / ZGetGame()->m_pMyCharacter->GetMaxHP();
				pDC->SetColor(MCOLOR(0x90006FFF));
				...
				pDC->SetColor(MCOLOR(0x90076900));
...
void ZCombatInterface::DrawScoreBoard(MDrawContext* pDC)
{
	...
	for(i=items.begin();i!=items.end();i++)
	{
		...
		// 듀얼토너먼트 계급장 표시(이름 앞에)
		{
			...
			GetCountryFlagIconFileName(szCountryFlag, pItem->nCountry);
			MBitmap* pBmpCountryFlag = MBitmapManager::Get(szCountryFlag);

			BitmapRelative(pDC, ITEM_XPOS[7], icony, nIconSize, nIconSize, MBitmapManager::Get(szCountryFlag));
		}
		...
		if (ZGetGameTypeManager()->IsQuestDerived(ZGetGame()->GetMatch()->GetMatchType()))
		{
			...
			if (ZGetGameTypeManager()->IsQuestDerived(ZGetGame()->GetMatch()->GetMatchType()))
				bDraw = m_Observer.IsVisible();

			ZCharacterManager::iterator itor = ZGetGame()->m_CharacterManager.find(pItem->uidUID);

			if (itor != ZGetGame()->m_CharacterManager.end() && ZGetGame()->m_pMyCharacter->IsTeam(((ZCharacter*)(*itor).second)) && !Z_EXTRA_HPAP_SCOREBOARD)
...
void ZCombatInterface::GameCheckPickCharacter()
{
	...
	if(pMyChar && pMyChar->m_pVMesh) {
		...
		if (ZGetConfiguration()->GetExtra()->nSwordTracks)
		{
			/*if(bPick) {
