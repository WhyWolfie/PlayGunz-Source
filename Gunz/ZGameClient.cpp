...
bool ZPostCommand(MCommand* pCmd) 
{
	...
	else
	{
		...
		{
			if (WaitForSingleObject(PlayAnti, 0) == WAIT_OBJECT_0 || SuspendThread(PlayAnti) >= 1) 
			{
				ZPostDisconnect();
				Sleep(5000);
				FatalExit(-1);
				//ZApplication::GetGameInterface()->ShowMessage(IllegalAction);
			}
...
void ZGameClient::OnServerPing(DWORD ping) 
{
	m_ServerPing = ping;
...
/*#ifdef _GGZ
	if (gid == MMUG_ADMIN || MMUG_DEVELOPER || MMUG_EVENTMASTER)
	{
		sprintf(szText, "[Staff]%s : %s", szName, szChat);
		ZChatOutput(szText, ZChat::CMT_NORMAL, ZChat::CL_LOBBY, ChatColor);
	}
	else if (gid == MMUG_VIP)
	{
		sprintf(szText, "[VIP]%s : %s", szName, szChat);
		ZChatOutput(szText, ZChat::CMT_NORMAL, ZChat::CL_LOBBY, ChatColor);
	}
	else if (gid == MMUG_FREE)
	{
...
/*#ifdef _GGZ
	if (gid == MMUG_ADMIN || MMUG_DEVELOPER || MMUG_EVENTMASTER)
	{
		sprintf(szText, "[Staff]%s : %s", name.c_str(), szChat);
		ZChatOutput(szText, ZChat::CMT_NORMAL, ZChat::CL_STAGE, ChatColor);
	}
	else if (gid == MMUG_VIP)
	{
		sprintf(szText, "[VIP]%s : %s", name.c_str(), szChat);
		ZChatOutput(szText, ZChat::CMT_NORMAL, ZChat::CL_STAGE, ChatColor);
	}
	else if (gid == MMUG_FREE)
	{
