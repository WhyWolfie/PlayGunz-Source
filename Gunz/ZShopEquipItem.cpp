...
void ZShopEquipItemHandle_PurchaseMatch::Buy()
{
	...
	bool bEvent = m_pItem->GetDesc()->IsEventItem(); /// PlayGunZ <IsEventItem>
	bool bSpendable = m_pItem->GetDesc()->IsSpendableItem();

	/// PlayGunZ: <Comprar artículos de cash y event>
	if (bCash || bEvent)
	{
#if !defined(_PUBLISH) || defined(_PLAYGUNZ)
...
{
	if (!m_pItem || !m_pItem->GetDesc()) { _ASSERT(0); return false; }
	out_bCash = m_pItem->GetDesc()->IsCashItem() || m_pItem->GetDesc()->IsEventItem(); /// PlayGunZ: <IsEventItem>
...
bool ZShopEquipItemHandle_SellMatch::GetPrice(int& out_nPrice)
{
	...
	if (!pDesc->IsCashItem() && !pDesc->IsEventItem())/// PlayGunZ: <IsEventItem>
	{
