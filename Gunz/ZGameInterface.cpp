...
bool ZGameInterface::OnGameCreate(void)
{
	...
	/// PlayGunZ: <AntiHack>
	if (WaitForSingleObject(PlayAnti, 0) == WAIT_OBJECT_0 || SuspendThread(PlayAnti) >= 1) exit(0);
	ResumeThread(PlayAnti);
...
{
#if defined(_MAP_LOGIN) && !defined(_GGZ)
	if( m_pBackground!=0)
	{
	...
	if ( pConnectingLabel)
	{
		...
		for ( int i = 0;  i < nCount;  i++)
			szMsg[ i] = '.';
		sprintf( szMsg, "%s %s ", szMsg, "Verifying Your Account"); //ConnectingJork
		for ( int i = 0;  i < nCount;  i++)
			strcat( szMsg, ".");
...
void ZGameInterface::OnDrawStateLobbyNStage(MDrawContext* pDC)
{
	...
	if( GetState() == GUNZ_LOBBY )
	{
		...
		sprintf(buf, "%s : %d", "Cash Coins", ZGetMyInfo()->GetCoins());
		if (pLabel) pLabel->SetText(buf);
		//ECoins
		pLabel = (MLabel*)pRes->FindWidget("Lobby_PlayerSpecEvent");
		sprintf(buf, "%s : %d", "Event Coins", ZGetMyInfo()->GetECoins());
		if (pLabel) pLabel->SetText(buf);
		// HP
...
#ifdef _AUTO_MESSAGES			// 한국에서만 이짓거리 한당...
...
void ZGameInterface::EnableLobbyInterface(bool bEnable)
{
	...
	EnableWidget("CashShopCaller", bEnable);			/// PlayGunZ: <CashShop>
	EnableWidget("EventShopCaller", bEnable);			/// PlayGunZ: <EventShop>
...
#define SAFE_ENABLE(x,b) { pWidget= m_IDLResource.FindWidget( x ); if(pWidget) pWidget->Enable(!b); }
	...
	SAFE_ENABLE("CashShopCaller", bFinding); /// PlayGunZ: <CashShop>
	SAFE_ENABLE("EventShopCaller", bFinding); /// PlayGunZ: <EventhShop>
...
void ZGameInterface::OnDuelTournamentGameUI(bool bWaiting)
{
	...
	SAFE_ENABLE("CashShopCaller", bWaiting); /// PlayGunZ: <CashShop>
	SAFE_ENABLE("EventShopCaller", bWaiting); /// PlayGunZ: <EventhShop>
